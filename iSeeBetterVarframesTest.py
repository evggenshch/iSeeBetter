...
parser.add_argument('-c', '--gpu_mode', action='store_true', required=False,
                    help="Use a CUDA compatible GPU if available")
...
test_set = get_test_set(args.data_dir, args.nFrames, args.upscale_factor, args.file_list, args.other_dataset,
                        args.future_frame)
testing_data_loader = DataLoader(dataset=test_set, num_workers=args.threads, batch_size=args.testBatchSize,
                                 shuffle=False)

default_model_n_frames = 7
...
def save_img(img, img_name, pred_flag):
    ...
    # save img
    save_dir = os.path.join(args.output, args.data_dir,
